...
def process_excel_file(
    ...
    try:
        ...
        with vlog.step("Saving original version"):
            ...
            tqdm.write(
                f"  ✓ Created original/{output_file.name} with {records_count} rows (original)"
            )
        ...
        with vlog.step("Cleaning duplicate columns"):
            ...
            vlog.detail(
                f"Removed {len(df.columns) - len(df_cleaned.columns)} duplicate columns"
            )

        with vlog.step("Saving cleaned version"):
            records_count_cleaned = convert_dataframe_to_jsonl(
                df_cleaned, output_file_cleaned, excel_file.name
            )
            vlog.detail(
                f"Created: {output_file_cleaned.name} ({records_count_cleaned} records)"
            )
            tqdm.write(
                f"  ✓ Created cleaned/{output_file_cleaned.name} with {records_count_cleaned} rows (cleaned)"
            )
        ...
        error_msg = f"Error processing {excel_file.name}: {e!s}"
...
def clean_duplicate_columns(df: pd.DataFrame) -> pd.DataFrame:
            ...
            if base_name in df.columns:
                try:
                    ...
                    if df[col].isna().all() or df[col].equals(df[base_name]):
                        ...
                        log.debug(
                            f"Marking {col} for removal (duplicate of {base_name})"
                        )
                        vlog.detail(
                            f"Marking {col} for removal (duplicate of {base_name})"
                        )
    ...
    if columns_to_remove:
        ...
        log.info(
            f"Removed {len(columns_to_remove)} duplicate columns: {', '.join(columns_to_remove)}"
        )
        vlog.detail(
            f"Removed {len(columns_to_remove)} duplicate columns: {', '.join(columns_to_remove)}"
        )
...
def extract_excel_to_jsonl() -> dict[str, Any]:
    ...
    log.debug(
        f"Excel files: {[f.name for f in excel_files[:10]]}{'...' if len(excel_files) > 10 else ''}"
    )
    ...
    with vlog.file_processing("Data extraction", total_records=len(excel_files)):
        ...
        for file_index, excel_file in enumerate(
            ...
            original_file = (
                Path(config.CLEAN_DATASET_DIR) / "original" / f"{excel_file.stem}.jsonl"
            )
            cleaned_file = (
                Path(config.CLEAN_DATASET_DIR) / "cleaned" / f"{excel_file.stem}.jsonl"
            )
            ...
            if (
                ...
                and check_file_integrity(original_file)
                and check_file_integrity(cleaned_file)
            ):
                files_skipped += 1
                tqdm.write(
                    f"  ⊙ Skipping {excel_file.name} (valid output already exists)"
                )
            ...
            if original_file.exists() or cleaned_file.exists():
                tqdm.write(
                    f"  ⚠ Re-processing {excel_file.name} (existing files are corrupted or incomplete)"
                )
                log.warning(
                    f"Re-processing {excel_file.name} - existing files corrupted"
                )
            ...
            with vlog.step(f"File {file_index}/{len(excel_files)}: {excel_file.name}"):
                success, records_count, error_msg = process_excel_file(
                    excel_file, config.CLEAN_DATASET_DIR
                )
                if success:
                    files_created += 1
                    total_records += records_count
                    log.debug(
                        f"Successfully processed {excel_file.name}: {records_count} records"
                    )
    ...
    log.info(
        f"Extraction complete: {total_records} records, {files_created} files created, {files_skipped} skipped"
    )
...
if __name__ == "__main__":
    ...
    if result["errors"]:
        ...
        log.success(
            f"Extraction successful: {result['files_created']} files created, {result['total_records']} records processed"
        )
        sys.exit(0)
