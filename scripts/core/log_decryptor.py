...
Examples:
    ...
    python -m scripts.core.log_decryptor encrypted_logs/ --since "1 day ago"
...
""",
    ...
    if args.generate_keys:
        ...
        if not password:
            ...
            password = getpass.getpass("Enter password for private key (empty for none): ")
    ...
    try:
        ...
        if args.since:
            ...
            if "ago" in args.since.lower():
                ...
                unit = parts[1]
                if "day" in unit:
                    since = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(days=amount)
                elif "hour" in unit:
                    since = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(hours=amount)
            else:
        ...
        if not args.show_phi:
            from scripts.core.structured_logging import _redact_phi
