...
Usage:
    ...
    >>> from client.react_rag_agent import ReActRAGAgent
...
class ReActRAGAgent:
    ...
    async def connect(self) -> None:
        ...
        self._connected = True
        print(f"✅ ReAct RAG Agent ready (max iterations: {self.config.max_iterations})")
        ...
        while state.should_continue and state.iteration < state.max_iterations:
            ...
            if response.get("tool_calls"):
                ...
                for tool_call in response["tool_calls"]:
                    ...
                    state.add_action(f"Called {tool_call['function']['name']}", tool_call)
    ...
    async def _execute_tool(self, tool_call: dict[str, Any]) -> ToolResult:
        ...
        if self._mcp_adapter:
            mcp_tool_names = [t["function"]["name"] for t in self._mcp_adapter.get_tools()]
    ...
    async def _search_variables(self, query: str) -> ToolResult:
        ...
        if results:
            ...
            for r in results[:15]:
                var_info = {
                    "variable": r.get("variable_name") or r.get("name") or r.get("field"),
            ...
            with open(target_file, encoding="utf-8") as f:
                for i, line in enumerate(f):
                    if line.strip():
                        record = json.loads(line)
                        var_info = {
                            "name": record.get("variable_name") or record.get("name") or record.get("field", f"field_{i}"),
                            "label": record.get("label") or record.get("description", ""),
                            "type": record.get("data_type") or record.get("type", ""),
                            "values": record.get("allowed_values") or record.get("codelist", ""),
                        }
    ...
    async def _get_study_overview(self) -> ToolResult:
        ...
        dict_files = list(self.dictionary_dir.glob("*.jsonl")) if self.dictionary_dir.exists() else []
        data_dirs = [d for d in self.deidentified_dir.glob("*") if d.is_dir()] if self.deidentified_dir.exists() else []
        ...
        overview = {
            ...
            "data_domains": [
                {"code": "DM", "name": "Demographics", "description": "Patient demographics and enrollment"},
                {"code": "LB", "name": "Laboratory", "description": "Lab tests including TB diagnostics"},
                {"code": "VS", "name": "Vital Signs", "description": "Physical measurements"},
                {"code": "MH", "name": "Medical History", "description": "Prior conditions and TB history"},
                {"code": "AE", "name": "Adverse Events", "description": "Treatment-related events"},
            ],
            "visit_schedule": [
                "Screening", "Baseline/Enrollment", "Month 2", "Month 6", 
                "Month 12", "Month 18", "Month 24", "End of Study"
            ],
            "available_resources": {
    ...
    async def _find_outcome_variables(self, outcome_type: str, cohort: str = "A") -> ToolResult:
        ...
        outcome_mappings = {
            ...
            "incident_tb": {
                ...
                "B": {
                    "variables": ["TBINCIDENT", "TBDIAG", "TST_CONVERT", "IGRA_CONVERT", "ACTIVETB"],
            ...
            "treatment_outcome": {
                "A": {
                    "variables": ["TXOUTCOME", "TXSTATUS", "CURED", "COMPLETED", "FAILED", "DEFAULTED"],
        ...
        outcome_info = outcome_mappings[outcome_key].get(cohort_key, outcome_mappings[outcome_key]["A"])
        ...
        result = {
            "cohort": f"Cohort {cohort_key}" + (" (TB index cases)" if cohort_key == "A" else " (Household contacts)"),
    ...
    async def _find_predictor_variables(self, category: str = "", concept: str = "") -> ToolResult:
        ...
        predictor_categories = {
            ...
            "clinical": {
                "variables": ["TBTYPE", "CAVITARY", "SMEAR", "CULTURE", "XRAY", "SEVERITY"],
        ...
        if category:
            ...
            if cat_key in predictor_categories:
                ...
                for term in cat_info["search_terms"][:3]:
                    ...
                    if search_result["success"] and "No variables found" not in search_result["result"]:
                        found.append(term)
            ...
            results["search_results"] = json.loads(search_result["result"]) if search_result["success"] else {}
    ...
    async def _recommend_visualization(
        ...
        categorical_vars = {"sex", "gender", "diabetes", "dm", "smoking", "alcohol", "hiv", "site"}
        binary_outcome_keywords = {"recurrence", "incident", "death", "relapse", "outcome"}
        ...
        for predictor in predictors:
            ...
            is_binary_outcome = any(bo in outcome_lower for bo in binary_outcome_keywords)
            ...
            if stratify_by:
                ...
                if is_continuous:
                    viz["stratified_plot"] = f"scatter plot colored by {'/'.join(stratify_by)}"
                else:
                    viz["stratified_plot"] = f"faceted violin plots by {'/'.join(stratify_by)}"
