...
Usage:
    >>> from reportalin.server.data_pipeline import DataPipelineConnector
    >>>
...
from reportalin.core.constants import MIN_K_ANONYMITY
from reportalin.core.exceptions import PrivacyViolationError
...
# Note: PrivacyViolationError is imported from reportalin.core.exceptions
...
REPORT_INDIA_DOMAINS = {
    ...
    "3_Specimen_Collection": (
        "SP",
        "Specimen Collection - Sputum, blood, other samples",
    ),
...
class DataPipelineConnector:
    ...
    async def get_dataset_schema(
        ...
        try:
            ...
            return {
                ...
                "source": "deidentified"
                if self._is_deidentified(file_path)
                else "extracted",
    ...
    def get_pipeline_status(self) -> dict[str, Any]:
        ...
        if self.extracted_dir.exists():
            status["extracted_file_count"] = len(
                list(self.extracted_dir.rglob("*.jsonl"))
            )

        if self.deidentified_dir.exists():
            status["deidentified_file_count"] = len(
                list(self.deidentified_dir.rglob("*.jsonl"))
            )
